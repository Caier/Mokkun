...
import { group, aliases, register, CmdParams as c, options } from "../../util/commands/CommandDecorators.js";
...
@group(CommandGroup.Interaction)
export default class {
    @aliases('select', 'sel', 'ch')
    @register('chooses one of the provided choices', '', { free: 0 })
    @options({ type: ApplicationCommandOptionType.String, name: 'options', description: 'the options to choose from separated by the | character', required: true })
    ...
    @options({ type: ApplicationCommandOptionType.Integer, name: 'max', description: 'the maximum roll', required: true },
             { type: ApplicationCommandOptionType.Integer, name: 'min', description: 'the minimum roll' })
